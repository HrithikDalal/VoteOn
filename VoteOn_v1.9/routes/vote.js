...
router.get("/blockchain",middleware.isLoggedIn,middleware.isAdmin,function(req,res){
    res.send(JSON.stringify(voteChain,null,5));
});
...
{            
   ...
   /* res.render("yourVote",{yourVote:yourVote[0]});*/
    Candidate.findOne({ publicAddress : yourVote[0].toAddress },function(err,candidate){
        if(err)
        {
            console.log(err);
            res.render("home");
        }
         res.render("yourVote",{candidate:candidate});
         });
});

router.get("/results",middleware.isLoggedIn,middleware.isAdmin,function(req,res){
 Candidate.find({}, function(err, allCandidates){
         if(err){
             console.log(err);
         }else {
